  ...
  return (
    <section className="w-full h-full">
      ...
      >
        ...
        <BlurFadeText
          className="my-5 text-start text-bold  w-full  text-black-60"
          yOffset={3}
          delay={0.4}
          text={aboutUsData?.welcomeMessage}
        />
      </motion.div>
      <div className="grid sm:grid-cols-2     lg:grid-cols-3 grid-cols-1 bg-white-10 max-w-[1280px] mx-auto gap-10 mb-5 px-4 ">
        {aboutUsData?.ourMission?.map((data, index) => (
          <motion.div
            ...
            viewport={{ once: true }}
          >
            <div className="text-center text-[18px] mb-2">{data?.title}</div>
            <p className="text-center text-[14px]">{data?.content}</p>
          </motion.div>
        ))}
      </div>
        ...
        <div className="grid grid-cols-1 w-full max-w-[1280px] mx-auto gap-10">
          {aboutUsData?.team?.map((data, index) => (
            ...
            >
              {index % 2 === 0  && (
                <div className="flex-1 md:p-2 flex flex-col items-center justify-center gap-3">
                  <img
                    ...
                    alt="profile-pic"
                  />
                  <p className="text-center text-bold text-[18px] text-black-30">
                  {data?.name}
                </p>
              ...
              {index % 2 !== 0 && (
                <div className="flex-1 md:p-2 flex-col items-center flex justify-center gap-3">
                  <img
                    src={"https://teclearns.com/images/edu-avatar.jpg"}
                    className="w-[167px] h-[165px]"
                    alt="profile-pic"
